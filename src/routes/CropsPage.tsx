  ...
  return (
    <main className="max-w-2xl mx-auto px-4 pb-16 pt-[max(1.5rem,env(safe-area-inset-top))]">
      ...
      {loading ? (
        ...
        <EmptyState
          emoji="🌾"
          title={t('noCropsTitle')}
          description={t('noCropsBody')}
          action={
        ...
        <>
          <div className="flex items-center justify-between mb-3">
            <SectionTitle>{t('growing')}</SectionTitle>
          ...
          {active.length === 0 ? (
            <Card>
              <p className="text-sm text-[var(--muted)]">{t('allArchived')}</p>
            </Card>
          ) : (
          ...
          {archived.length > 0 ? (
            <div className="mt-8">
              <SectionTitle>{t('archivedSection')}</SectionTitle>
              <ul className="space-y-2.5">
                {archived.map((crop) => (
                  <li key={crop.id}>
                    <CropRow crop={crop} />
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-[var(--faint)] mt-1.5">
        {formatLongDate(crop.startDate, locale)} ·{' '}
        {t('members', { count: crop.members.length })}
      </p>
    </Link>
  )
}
